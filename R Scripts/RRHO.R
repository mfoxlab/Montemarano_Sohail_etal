...
# Get all objects in the environment that end in 'RRHO'
rrho_objects <- ls(pattern = "RRHO$")

# Loop through each RRHO object
for (obj_name in rrho_objects) {
  # Extract the object by its name
  rrho_object <- get(obj_name)
  
  # Define a list of the gene lists and their suffixes
  genelists <- list(
    uu = rrho_object$genelist_uu,
    dd = rrho_object$genelist_dd,
    ud = rrho_object$genelist_ud,
    du = rrho_object$genelist_du
  )
  
  # Loop through each gene list
  for (suffix in names(genelists)) {
    genelist <- genelists[[suffix]]
    
    # Export the 3 sublists as .txt files
    for (i in 1:length(genelist)) {
      file_name <- paste0(obj_name, "_", suffix, "_", i, ".txt")
      write.table(genelist[[i]], file = file_name, sep = "\t", row.names = FALSE, col.names = FALSE)
    }
  }
}
