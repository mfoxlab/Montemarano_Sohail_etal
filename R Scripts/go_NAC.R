...
workdir = "/Users/hsohail/OneDrive - Penn State Health/Documents/WGCNA/Annalisa/New NAC"
setwd(workdir)
...
top_terms <- NAC_go_all_modules_cleaned %>%
  group_by(module) %>%
...
ggplot(top_terms, aes(x = as.factor(module), 
  ...
  labs(title = "GO Enrichment Dot Plot: NAC",
       x = "Module",
       y = "GO Term",
       size = "# Genes in Term",
       color = "-log10(p-value)") +
...
write.table(NAC_go_all_modules_cleaned, file = "NAC_go_masterlist_concat.txt", row.names = FALSE, sep = '\t', col.names = TRUE, quote = FALSE)
