...
load(file= "logcpm_AMY_sig.Rdata")
...
powerthres = 8
netAMY_8 = blockwiseModules(logcpm_AMY_sig, corType = "bicor", maxPOutliers = 0.1, 
                              power = powerthres, networkType = "signed", 
                              minModuleSize = 30, reassignThreshold = 0,
                              mergeCutHeight = 0.25, numericLabels = TRUE, minMEtoStay = 0, pamRespectsDendro = FALSE, 
                              saveTOMs = TRUE, saveTOMFileBase = "8TOM.AMY",
                              loadTOM = T, verbose = 5)



save(netAMY_8, file = "netAMY_8.Rdata")
...
AMY_module_colors<- netAMY_8$colors
...
MEsAMY = moduleEigengenes(
  ...
  align = "along average",
  excludeGrey = FALSE,
  grey = if (is.numeric(AMY_module_colors))
    0
  else
    "grey",
  subHubs = TRUE,
...
netGenes = data.frame(ensembl_gene_id = colnames(logcpm_AMY_sig), module.AMY = netAMY_8$colors)
...
#sex related 
sex_combined_df <- cbind(MEsAMY$eigengenes, Sex = sex_data$Sex)
...
sex_drug_combined_df <- cbind(MEsAMY$eigengenes, SexCondition = sex_condition_combined)
...
female_eigengenes <- t(female_data[, 1:ncol(MEsAMY$eigengenes)])
female_lm_fit <- lmFit(female_eigengenes, female_design)

male_design <- model.matrix(~0 +SexCondition, data = male_data)
male_eigengenes <- t(MEsAMY$eigengenes[rownames(male_data), ])
...
write.table(results_annotated, "AMY_sexcomb_moduleEigengenesDE.txt", sep = "\t", row.names = FALSE, quote = FALSE) #module eigengene data for sexes combined
write.table(female_results, "AMY_F_moduleEigengenesDE.txt",sep="\t", row.names=FALSE, quote=FALSE) #module eigengene data for females
write.table(male_results,"AMY_M_moduleEigengenesDE.txt", sep="\t", row.names=FALSE, quote=FALSE) #module eigengene data for males

#now with sexes separated 

save.image(file = "WGCNA_AMY_workspace.RData")
