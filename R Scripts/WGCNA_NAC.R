...
powerthres = 8
netNAC_8 = blockwiseModules(logcpm_NAC_sig, corType = "bicor", maxPOutliers = 0.1, 
                            power = powerthres, networkType = "signed", 
                            minModuleSize = 30, reassignThreshold = 0,
                            mergeCutHeight = 0.25, numericLabels = TRUE, minMEtoStay = 0, pamRespectsDendro = FALSE, 
                            saveTOMs = TRUE, saveTOMFileBase = "8TOM.NAC",
                            loadTOM = T, verbose = 5)



save(netNAC_8, file = "netNAC_8.Rdata")
...
NAC_module_colors<- netNAC_8$colors
...
MEsNAC = moduleEigengenes(
  ...
  align = "along average",
  excludeGrey = FALSE,
  grey = if (is.numeric(NAC_module_colors))
    0
  else
    "grey",
  subHubs = TRUE,
...
DEsNAC <- read.table(paste0(workdir, "/DEgene_summary_NAC.txt"), sep = "\t", header=TRUE)
...
netGenes = data.frame(ensembl_gene_id = colnames(logcpm_NAC_sig), module.NAC = netNAC_8$colors)
...
#sex related 
sex_combined_df <- cbind(MEsNAC$eigengenes, Sex = sex_data$Sex)
...
sex_drug_combined_df <- cbind(MEsNAC$eigengenes, SexCondition = sex_condition_combined)
...
female_eigengenes <- t(female_data[, 1:ncol(MEsNAC$eigengenes)])
female_lm_fit <- lmFit(female_eigengenes, female_design)

male_design <- model.matrix(~0 +SexCondition, data = male_data)
male_eigengenes <- t(MEsNAC$eigengenes[rownames(male_data), ])
...
write.table(results_annotated, "NAC_sexcomb_moduleEigengenesDE.txt", sep = "\t", row.names = FALSE, quote = FALSE) #module eigengene data for sexes combined
write.table(female_results, "NAC_F_moduleEigengenesDE.txt",sep="\t", row.names=FALSE, quote=FALSE) #module eigengene data for females
write.table(male_results,"NAC_M_moduleEigengenesDE.txt", sep="\t", row.names=FALSE, quote=FALSE) #module eigengene data for males
