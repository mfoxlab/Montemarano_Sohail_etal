...
module_info <- read.table("NAC_circosdata.txt", sep = '\t', header = TRUE, quote = '')
...
colnames(module_info_logp)[5:7] <- c("logP.Value_NAC_FENT", "logP.Value_NAC_F_FENT", "logP.Value_NAC_M_FENT")
module_info_logp <- module_info_logp[order(module_info_logp$logFC_NAC_FENT), ]
...
circos.clear()
pdf("circosplot_NAC.pdf", width = 9.5, height = 7)
...
pdf("circosplot_NAC_labeled.pdf", width = 9.5, height = 7)
circos.par(start.degree = -180, gap.degree=0.2)
...
# Load the module information and module color data
module_info <- read.table("NAC_circosdata_all_MEs.txt", sep = '\t', header = TRUE, quote = '')
color_data <- read.delim(file.path(colordir, "modulecolors.txt"), header = TRUE, stringsAsFactors = FALSE, quote = '', fill = TRUE)
...
circos.clear()
pdf("circosplot_NAC_all_MEs.pdf", width = 9.5, height = 7)
circos.par(start.degree = -180, gap.degree=0.2)
...
pdf("circosplot_NAC_all_MEs_labeled.pdf", width = 9.5, height = 7)
circos.par(start.degree = -180, gap.degree=0.2)
...
# Save the workspace --------------------------------------------------------------------------------
save.image(file = paste0(workdir, "/circosplotNACworkspace.Rdata"))
