...
workdir = "/Users/hsohail/OneDrive - Penn State Health/Documents/WGCNA/Annalisa/New INPUT/Circos"
colordir = "/Users/hsohail/OneDrive - Penn State Health/Documents/WGCNA/Annalisa/New WGCNA"
setwd(workdir)
load(file = paste0(workdir,"/circosplotINPUTworkspace.Rdata"))
...
module_info <- read.table("INPUT_circosdata.txt", sep = '\t', header = TRUE, quote = '')
...
# Log base 10 transform columns 5-7 (P values)
module_info_logp <- module_info
module_info_logp[, 5:7] <- -log10(module_info[, 5:7])
colnames(module_info_logp)[5:7] <- c("logP.Value_INPUT_FENT", "logP.Value_INPUT_F_FENT", "logP.Value_INPUT_M_FENT")
...
# PLOT 1: SIGNIFICANT MODULES, UNLABELED --------------------------------------------------------------------------------
circos.clear()
pdf("circosplot_INPUT.pdf", width = 9.5, height = 7)
...
# Plot the first three tracks for logFC columns (columns 2, 3, 4)
for (i in 2:4) {
...
# Plot the next three tracks for P-value columns (-log10(P value) transformed in columns 5, 6, 7)
for (i in 5:7) {
  circos.track(sectors = module_info_logp$ModuleEigengene, ylim = c(0, 1), track.height = 0.05,
...
# PLOT 2: ALL  MODULES, LABELED --------------------------------------------------------------------------------
pdf("circosplot_INPUT_labeled.pdf", width = 9.5, height = 7)
circos.par(start.degree = -180, gap.degree=0.2)
...
# PLOT 3: ALL MODULES, UNLABELED --------------------------------------------------------------------------------
# Load the module information and module color data
module_info <- read.table("INPUT_circosdata_all_MEs.txt", sep = '\t', header = TRUE, quote = '')
color_data <- read.delim(file.path(colordir, "modulecolors.txt"), header = TRUE, stringsAsFactors = FALSE, quote = '', fill = TRUE)
...
circos.clear()
pdf("circosplot_INPUT_all_MEs.pdf", width = 9.5, height = 7)
circos.par(start.degree = -180, gap.degree=0.2)
...
# PLOT 4: ALL MODULES, LABELED --------------------------------------------------------------------------------
pdf("circosplot_INPUT_all_MEs_labeled.pdf", width = 9.5, height = 7)
circos.par(start.degree = -180, gap.degree=0.2)
...
# Save the workspace --------------------------------------------------------------------------------
save.image(file = paste0(workdir, "/circosplotINPUTworkspace.Rdata"))
